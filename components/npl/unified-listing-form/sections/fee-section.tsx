 ...
 *   · 전속 OFF (일반) : 0.7% ~ 0.9% 범위에서 자유 설정
 *   · 전속 ON         : 0.5% 혜택 자동 적용 (+ 조선일보 땅집고 기사 지원)
 *
 * Phase G5 (최초):
 *   · NPLatform 전속 계약 토글을 InstitutionSection → FeeSection 최상단으로 이동.
 *
 * Phase G6+ (2026-04-24, 수수료 정책 반전):
 *   · 전속 ON 혜택 : 기존 "하한 0.5% 상향" → "0.5% 자동 적용 + 땅집고 기사 지원"
 *   · 일반 매물   : 기존 0.5%~0.9% → 0.7%~0.9%
...
const EXCLUSIVE_RATE = 0.005          // 전속 계약 자동 적용 요율 (0.5%)
const GENERAL_MIN_RATE = 0.007        // 일반 매물 하한 (0.7%)
...
export function FeeSection({
  ...
  // Phase G6+ 정책:
  //   · 전속 ON 전환 → 0.5% 혜택 자동 적용 (기존 rate 와 무관하게 혜택값으로 설정)
  //   · 전속 OFF 전환 → rate 가 일반 하한(0.7%) 미만이면 0.7% 로 자동 상향 보정
          ...
          <label className="flex items-start gap-3 cursor-pointer">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[0.6875rem] text-[var(--color-text-tertiary)] leading-relaxed">
                ...
                <strong className="text-stone-900 dark:text-stone-900"> 수수료는 0.5% 로 조정</strong>됩니다.
