...
export function SampleBadge({ mode = "compact", className }: SampleBadgeProps) {
  ...
  if (mode === "full") {
    return (
      <div
        className={cn(
          "inline-flex items-center gap-1.5 rounded-md border border-amber-300 bg-amber-50 px-3 py-1.5 text-sm font-medium text-amber-800 dark:border-amber-700 dark:bg-amber-950/50 dark:text-amber-300",
          className
        )}
      >
  ...
  return (
    <Badge
      variant="outline"
      className={cn(
        "border-amber-300 bg-amber-50 text-amber-700 text-[10px] px-1.5 py-0 font-medium dark:border-amber-700 dark:bg-amber-950/50 dark:text-amber-400",
        className
      )}
    >
      샘플
