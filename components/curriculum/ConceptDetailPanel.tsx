    ...
    <div className="w-80 bg-white border-l border-gray-200 overflow-y-auto flex-shrink-0">
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          <span className="px-2 py-0.5 bg-gray-100 text-gray-700 text-xs rounded-full">{detail.level}</span>
            ...
            <div className="space-y-2">
              {importance.experts.map((exp, i) => (
                <div key={i} className="flex items-center gap-2">
                  <span className="text-xs text-gray-600 w-20 truncate">{exp.channel_name}</span>
        ...
        {prerequisites.length > 0 && (
          <div>
            <h4 className="text-xs font-semibold text-gray-500 mb-1">선수 개념</h4>
            <div className="space-y-1">
              {prerequisites.map((p: any) => (
                <div key={p.concept_id} className="flex items-center gap-1 text-sm text-purple-600 hover:text-purple-800 cursor-pointer">
                  <ChevronRight className="w-3 h-3" />
                  {p.name}
                  <span className="text-xs text-gray-400 ml-1">{p.level}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {relatedConcepts.length > 0 && (
          <div>
            <h4 className="text-xs font-semibold text-gray-500 mb-1">관련 개념</h4>
            <div className="flex flex-wrap gap-1">
              {relatedConcepts.slice(0, 8).map((c: any) => (
                <span key={c.concept_id} className="px-2 py-0.5 bg-blue-50 text-blue-600 text-xs rounded">
                  {c.name}
                </span>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {detail.youtube.slice(0, 5).map((yt: any) => (
                <div key={yt.youtube_id} className="bg-gray-50 rounded p-2">
                  <p className="text-xs font-medium text-gray-700 line-clamp-2">{yt.title}</p>
            ...
            <div className="space-y-1 max-h-40 overflow-y-auto">
              {atomicCapsules.slice(0, 10).map((cap) => (
                <Link
                  ...
                  className="flex items-center gap-2 p-1.5 rounded-lg hover:bg-purple-50 text-xs transition-colors group"
                >
                  ...
                  <span className="text-gray-700 truncate flex-1 group-hover:text-purple-700">{cap.topic}</span>
                  <span className="text-[9px] text-gray-400 shrink-0">{cap.estimated_min}분</span>
                </Link>
              ))}
              {atomicCapsules.length > 10 && (
                <p className="text-[10px] text-gray-400 text-center py-1">+{atomicCapsules.length - 10}개 더...</p>
              )}
            </div>
          </div>
        )}
        ...
        <div className="space-y-2">
          ...
          <Link
            ...
            className="block w-full text-center py-2 bg-purple-50 text-purple-700 text-sm font-medium rounded-lg hover:bg-purple-100 transition-colors"
          >
