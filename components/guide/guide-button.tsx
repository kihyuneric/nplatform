...
export function GuideButton({ serviceKey, label = "사용 가이드", variant = "default", theme = "light" }: GuideButtonProps) {
      ...
      <button
        ...
        className={`inline-flex items-center gap-1 text-xs transition-colors ${
          theme === "dark"
            ...
            : "text-muted-foreground hover:text-[#1B3A5C] dark:hover:text-blue-400"
        }`}
        title="사용 가이드 열기"
      >
  ...
  return (
    <Button
      ...
      size="sm"
      onClick={openGuide}
      className="h-8 gap-1.5 text-xs dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800"
    >
      <BookOpen className="h-3.5 w-3.5" />
      {label}
    </Button>
