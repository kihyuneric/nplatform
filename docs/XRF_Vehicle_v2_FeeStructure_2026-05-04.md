...
**Owner**: 트랜스파머 (KR Platform Co · KOF) + XRF Foundation
...
## 0. 글로벌 친화 용어 정의 (v6 · 2026-05-04)

| 신용어 (Global) | 약어 | 舊 용어 (KR) | 역할 |
|----------------|------|-------------|------|
| **XRF Foundation** | XRF | XRF (변경 없음) | SG SPV · RWA Issuer · Carry 수령자 |
| **Korea Operation Firm** | KOF | 엔플랫폼 | Korea PM Co · AI Valuation·Sourcing·PM·KR Margin 운영사 |
| **NPL Vehicle Company** | NPL VC | 대부업체 | Korea NPL 라이선스 보유 · 채권 보관·회수 SPV |
| **NPL VC Loan** | — | 대부업체 자본금/차입금 | LP→NPL VC 무이자 대여 (Day Exit 100% 환급 · 수수료 아님) |

**왜 용어를 바꾸었나?** XRF 글로벌 LP 가 "엔플랫폼", "대부업체" 라는 한국 특수 용어로는 그 역할을 직관적으로 이해할 수 없습니다. KOF 와 NPL VC 는 영문 약어로 cap table·IR pitch 어디서나 통용되도록 표준화한 명칭입니다.
...
|------|----------|-----------------------|-------|
| **LP 청약 모델** | LP 90% + NPL VC 10% Capital | **LP 100% Pool 청약 + 10% NPL VC 차입금** | LP가 Pool 전체 모금 |
| **NPL VC 자본 출자** | Pool의 10% (Capital, Fee 아님) | **LP→NPL VC 무이자 대여 10%** | 출자 → 차입금 모델로 전환 |
| **NPL VC 보상** | Servicing Fee 2.0%/yr + 자본금 1:1 환원 | **Servicing Fee 2.0%/yr (차입금에는 수수료/이자 無)** | Day Exit 시 차입금 1:1 환급 |
| **KOF BASE Fee 합계** | 3.00%/yr | **2.50%/yr** | -0.50%/yr |
| **0.5% 절감분 귀속** | (해당 없음) | **LP 순수익으로 자동 흡수** | LP profit ↑ |

---

## 2. KOF (KR Platform Co) Fee 재구성
...
> Hierarchy 는 Setup + Carry + KOF 컴포넌트로 형성됨.
...
> ⚠ **XRF Carry 조건부 수령 (중요)**: XRF Foundation 의 Carry (성과보수) 는 LP 가
> **Hurdle Rate 8%/yr × LP capital × 운용기간** 달성 시 그 초과분에 대해서만 적용됩니다.
> Hurdle 미달 시 Carry = $0 (XRF는 Mgmt 0.5% + Setup 만 수령).
> 즉 LP 가 우선적으로 8% 연환산 수익을 보장받은 후, 잉여분의 일부만 XRF 가 가져갑니다.

### 3.2 KOF 보수 — tier 별 변동 (v3 · 2026-05-04 사용자 피드백 반영)

> ⚠ **v3 변경 사유**: v2 에서 CONS KOF = BASE (Carry 만 양보) 라 수수료가 동일했음.
...
| **KOF 합계** | **2.50%/yr** | **2.25%/yr** | **2.00%/yr** |

### 3.3 NPL VC (KR Servicer)
...
elif BASE LP ROI ≥  5%  → SAVE-THE-DEAL (모두 양보 · XRF Carry 5% + KOF 2.0%/yr)
...
| Tier | XRF 수수료 | KOF 수수료 | LP ROI | 1인당 청약 |
|------|---------|--------------|--------|-----------|
...
> ✅ XRF 관리보수 0.5% flat (운영비) — Hierarchy 는 XRF Setup·Carry + KOF 4종이 형성
> ✅ Jongno BASE LP ROI 42.19% ≥ 20% → AUTO 판정 **BASE** (양보 불필요)

### v4 다른 case 검증

| Case | 차주 | XRF gradient | KOF gradient | AUTO |
|------|------|--------------|-------------------|------|
...
- **장점**:
  ...
  - KOF fee 0.5%p 절감 효과를 LP가 흡수
  - NPL VC 자본 라인 제거 → cap table 단순화
- **단점**:
...
- [x] `app/(main)/analysis/report/components/xrf-valuation-section.tsx`:
  - EXHIBIT 2 POOL 구조 — NPL VC 자본금 row 제거 · LP 100% 청약 표기
  - EXHIBIT 3 Vehicle Fee — KOF v2 % 라벨
  - EXHIBIT 4 Cash Flow — NPL VC 자본 라인 제거
  - 모델 설명 — NPL VC 자본 출자 폐지 명시
...
2. **LP 청약 시뮬레이션 Excel**:
   - Sheet "Fee Calculator" — KOF 4종 % 셀 갱신
