...
const EXCLUSIVE_CHOICES = ['전속 ON (수수료 0.5% + 땅집고옥션 마케팅 지원)', '전속 OFF (0.7%~0.9% 자유)']
...
addSelectRow('NPLatform 전속계약 (드롭다운 선택)', EXCLUSIVE_CHOICES, '전속 ON 시 수수료 0.5% 자동 + 땅집고옥션 마케팅 지원')
addBlank()
...
addInputRow('희망 수수료율 (%)', '', '일반 0.7~0.9 · 전속 시 0.5 자동 적용 (빈칸 허용)', null, FMT_PCT2)
...
console.log(`   v3.4 · 2시트만 (기본정보 + 특수조건) · 전속 0.5% + 땅집고옥션 마케팅 지원`)
