 ...
 *
 * 매물등록 엑셀 템플릿 생성 스크립트 (Phase G7+ · v3.2 · 드롭다운+O/X+수식).
 *
 * v3.2 변경 (2026-04-28):
...
let r = 1
s1.getRow(r).values = ['[시트 1] 매물 기본 정보 — NPL 매물등록 템플릿 v3.2 (드롭다운+O/X+수식)']
...
addFormulaRow(
  '수익권금액 (원) · 자동 · 최초원금 ×140%',
  `B${refs.initial_principal}*1.4`,
  '1차 근저당 수익권금액 = 최초 대출원금 × 110~140% (표준 140%)',
  FMT_MONEY,
  'beneficial_amount',
)
...
const guide = [
  ['[시트 4] 입력 가이드 · OCR 규격 (v3.2 · 드롭다운 + O/X + 자동계산 수식)'],
  [''],
  ['1. 입력 형식 구분'],
...
console.log(`   v3.2 · 드롭다운 + O/X + 자동계산 수식 · 4시트 (ExcelJS)`)
console.log(`   수식: 채권잔액·수익권금액·매각가A/B·계약금·LTV·권리관계점수`)
