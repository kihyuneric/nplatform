 ...
 *
 * 회차 산출: ceil(1 + (100 − 낙찰가율%) / 20)
 *   - 낙찰가율 ≥ 100% → 1회차 즉시 낙찰
 *   - 71.4% → 3회차 (1차 100, 2차 80, 3차 64% 시점)
 *
...
export function predictedAuctionRound(bidRatioPercent: number): number {
...
export function computeEffectiveFirstSaleDate(
  ...
  const round = predictedAuctionRound(expectedBidRatio * 100)
