...
const STATUS_STYLES = {
  ...
  in_progress: { icon: Play, color: 'text-stone-900', bg: 'bg-stone-100' },
  completed: { icon: CheckCircle2, color: 'text-stone-900', bg: 'bg-stone-100' },
  mastered: { icon: Star, color: 'text-stone-900', bg: 'bg-stone-100' },
}
    ...
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="flex items-center gap-2 mb-0.5">
            ...
            <span className="text-xs text-stone-900">{concept?.level}</span>
          </div>
        ...
        <div className="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-xl p-5 border border-stone-300">
          <div className="grid grid-cols-3 gap-4 mb-4">
            <div className="text-center">
              ...
              <div className="text-xs text-stone-900">총 캡슐</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-stone-900">{masteredCount}</div>
              <div className="text-xs text-stone-900">마스터 완료</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-stone-900">
                {totalMin >= 60 ? `${Math.floor(totalMin / 60)}h ${totalMin % 60}m` : `${totalMin}분`}
              </div>
              <div className="text-xs text-stone-900">총 학습시간</div>
            </div>
          </div>
          ...
          {progressPct === 100 && (
            <div className="mt-3 flex items-center gap-2 text-stone-900 text-sm font-medium">
      ...
      {capsules.length === 0 && (
        <div className="bg-white border-2 border-dashed border-stone-300 rounded-xl p-8 text-center space-y-4">
          ...
          {genProgress && (
            <div className="flex items-center gap-2 justify-center text-sm text-stone-900">
          ...
          <button
            ...
            disabled={generating}
            className="inline-flex items-center gap-2 px-6 py-3 bg-stone-100 text-white rounded-xl text-sm font-semibold hover:bg-stone-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={handleGenerate}
              disabled={generating}
              className="text-xs text-gray-400 hover:text-stone-900 flex items-center gap-1"
            >
          ...
          {genProgress && (
            <div className="flex items-center gap-2 text-sm text-stone-900 py-2">
              <Loader2 className="w-4 h-4 animate-spin" />
              {genProgress}
            </div>
          )}
            ...
            return (
              <Link
                ...
                className={`block p-4 rounded-xl border transition-all hover:shadow-sm ${
                  status === 'mastered' ? 'border-stone-300 bg-stone-100/40' :
                  status === 'completed' ? 'border-stone-300 bg-stone-100/30' :
                  status === 'in_progress' ? 'border-stone-300 bg-stone-100/20' :
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-0.5">
                      ...
                      {cap.generation_stage === 'stage2' && (
                        <span className="text-[10px] text-stone-900">🌐</span>
                      )}
                    </div>
                  ...
                  <div className="shrink-0 text-right">
                    ...
                    {cap.progress?.quiz_score !== null && cap.progress?.quiz_score !== undefined && (
                      <div className="text-xs mt-0.5">
                        <span className={`font-medium ${cap.progress.quiz_score >= 70 ? 'text-stone-900' : 'text-stone-900'}`}>
        ...
        <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-xl p-5 border border-stone-300">
          ...
          <div className="flex gap-2">
            <a
              ...
              className="inline-flex items-center gap-1.5 px-4 py-2 bg-stone-100 text-white rounded-lg text-sm font-medium hover:bg-stone-100 transition-colors"
            >
            ...
            <a
              ...
              className="inline-flex items-center gap-1.5 px-4 py-2 bg-white text-stone-900 border border-stone-300 rounded-lg text-sm font-medium hover:bg-stone-100 transition-colors"
            >
              <FileText className="w-4 h-4" />
              PDF 다운로드
            </a>
      ...
      <div className="border-t border-gray-100 pt-4">
        <Link
          ...
          className="flex items-center gap-2 text-sm text-gray-500 hover:text-stone-900"
        >
