...
const TYPE_META: Record<InstType, { label: string; color: string }> = {
  ...
  CAPITAL: { label: "캐피탈",    color: "#A855F7" },
  AMC:     { label: "AMC",      color: "#10B981" },
  INSURER: { label: "보험",      color: "#F59E0B" },
  SAVINGS: { label: "저축은행",  color: "#EF4444" },
}

const GRADE_META: Record<InstGrade, { color: string; bg: string }> = {
  S: { color: "#FBBF24", bg: "#FBBF241F" },
  A: { color: "#10B981", bg: "#34D3991F" },
  B: { color: "#3B82F6", bg: "#60A5FA1F" },
  C: { color: "#475569", bg: "#94A3B81F" },
}

const STATUS_META: Record<InstStatus, { label: string; color: string }> = {
  ACTIVE:    { label: "활성",     color: "#10B981" },
  PROBATION: { label: "관찰",     color: "#F59E0B" },
  SUSPENDED: { label: "정지",     color: "#EF4444" },
}
                ...
                <div key={m.id} style={{ padding:"10px 12px", borderRadius:8, backgroundColor:C.bg3, border:`1px solid ${C.bg4}`, display:"flex", alignItems:"center", gap:10 }}>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:2 }}>
                      {m.role==="MASTER" && <Crown size={11} color="#FBBF24" />}
                      ...
                      {m.status==="PENDING" && (
                        <span style={{ fontSize:9, padding:"2px 6px", borderRadius:999, backgroundColor:"#FFF7ED", color:"#B45309", border:"1px solid #FDE68A", fontWeight:800 }}>승인대기</span>
                      )}
                    </div>
