...
const DOC_STYLE: Record<typeof DOCS[number]["status"], { fg: string; bg: string; label: string; icon: typeof CheckCircle2 }> = {
  ok:     { fg: "#051C2C", bg: "rgba(5, 28, 44,0.10)", label: "공유됨", icon: CheckCircle2 },
  draft:  { fg: "#051C2C", bg: "rgba(5, 28, 44,0.10)", label: "초안",   icon: PenLine },
  signed: { fg: "#051C2C", bg: "rgba(5, 28, 44,0.10)", label: "서명",   icon: Shield },
}
    ...
    <section style={{ backgroundColor: "#080F1E", padding: "6rem 0", position: "relative", overflow: "hidden" }}>
      <div
        style={{
          ...
          background: "radial-gradient(circle, rgba(5, 28, 44,0.06) 0%, transparent 70%)",
          filter: "blur(80px)",
        }}
      />
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="snap-start shrink-0 basis-[90%] lg:basis-auto lg:shrink lg:col-span-2 flex flex-col min-w-0">
              ...
              <div className="flex-1 p-5 space-y-4 min-h-[380px]" style={{ background: "rgba(0,0,0,0.12)" }}>
                {MSGS.map((m, i) => {
                  ...
                  const accent = isBuyer ? "#051C2C" : m.from === "seller" ? "#051C2C" : "#051C2C"
                  return (
                    <motion.div
                      key={i}
                        ...
                        style={{
                          background: isSystem
                            ...
                            : isBuyer
                              ? "rgba(5, 28, 44,0.10)"
                              : "rgba(5, 28, 44,0.08)",
                          border: `1px solid ${isSystem ? "rgba(5, 28, 44,0.20)" : isBuyer ? "rgba(5, 28, 44,0.22)" : "rgba(5, 28, 44,0.20)"}`,
                        }}
                      >
        ...
        <div className="mt-10 flex items-center justify-center gap-3 flex-wrap">
          <Link
            ...
            style={{ background: "linear-gradient(135deg, #A78BFA, #7C3AED)", color: "white", boxShadow: "0 4px 14px rgba(5, 28, 44,0.25)" }}
          >
