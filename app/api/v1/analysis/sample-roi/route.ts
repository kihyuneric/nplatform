...
interface RoiBundle {
  /** 보고서 RECENT PIPELINE / 카드에 표시할 prominent ROI (권고 시나리오) */
  primary: number | null
  /** 비교용 — 모든 ROI 후보 */
  candidates: {
    /** strategies.recommended.roi — 권고 시나리오 ROI (보고서 핵심 의사결정 값) */
    recommendedScenario: number | null
    /** strategies.conservative.roi — 보수적 시나리오 */
    conservativeScenario: number | null
    /** strategies.aggressive.roi — 강세 시나리오 */
    aggressiveScenario: number | null
    /** investment.roi — 입력 시나리오 ROI */
...
function pickRoiBundle(report: ReturnType<typeof buildJongnoSampleReport>): RoiBundle {
  ...
  // 우선순위: strategies.recommended (보고서 핵심 의사결정) > investment (fallback)
  const primary = (typeof recommended === 'number' && Number.isFinite(recommended))
    ? recommended
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      note: '분석 대시보드 ROI = strategies.recommended.roi (권고 시나리오) — 보고서 핵심 의사결정 값',
    }, {
      headers: {
